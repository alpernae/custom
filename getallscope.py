...
def parse_args():
    ...
    parser.add_argument('-o', '--output', metavar='FILE', help='Specify output file with extension .txt or .json')
...
try:
    ...
    if output_file:
        ...
        if ext.lower() == '.json':
            ...
            print("Unsupported file extension. Use .txt or .json", file=sys.stderr)
            sys.exit(1)
    ...
    if output_file:
        if output_format == 'json':
            ...
            with open(output_file, 'w') as f:
                for handle, details in output_data["programs"].items():
                    f.write(f"[{handle}]\n")
                    for url in sorted(d['domain'] for d in details['urls'] if d['eligible']):
                        f.write(f"{url}\n")
                    for wildcard in sorted(d['domain'] for d in details['wildcards'] if d['eligible']):
                        f.write(f"{wildcard}\n")
                    f.write("\n")
